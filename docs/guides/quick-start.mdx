...
<Steps>
  ...
  <Step title="Run NeoGPT">
    ...
    ```bash Terminal
    Using Llamacpp to load TheBloke/Mistral-7B-Instruct-v0.1-GGUF.

    NeoGPT 🤖 is ready to chat. Type /exit to quit.
...
```
<Tip>
You can add `--recurisive` flag to the `--build` command to recursively scrapes all the links in the `builder.url` file.
</Tip>
